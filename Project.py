...
def getmarks():
    ...
    sql = (
        f"select * from marks where sclass={sclass} and ssec='{sec}' order by sname asc"
    )
    cur.execute(sql)
    res = cur.fetchall()
...
def getstud():
    ...
    sql = f"select * from student where class={sclass}, ssec='{sec}' order by sname"
    cur.execute(sql)
    res = cur.fetchall()
...
def disstudmarkclass():
    ...
    sclass = int(input("Enter the class in numbers:"))
    head = ["Name", "Admn", "Class", "Sec", "sub1", "sub2", "sub3", "sub4", "sub5"]
    sql = f" select * from marks where sclass={sclass} order by ssec , name "
    cur.execute(sql)
    res = cur.fetchall()
...
def disstudmarkclasssec():
    ...
    sec = input("Enter the section:")
    head = ["Name", "Admn", "Class", "Sec", "sub1", "sub2", "sub3", "sub4", "sub5"]
    sql = f" select * from marks where sclass={sclass} and ssec='{sec}' order by sname"
    cur.execute(sql)
    res = cur.fetchall()
    print(tabulate(res, headers=head))
...
def disstudbioclass():
    ...
    sclass = int(input("Enter the class in numbers:"))
    head = ["Admn", "Name", "Father name", "Mother Name", "Class", "Sec"]
    sql = f" select * from student where sclass={sclass} order by ssec , sname"
    cur.execute(sql)
    res = cur.fetchall()
    print(tabulate(res, headers=head))
...
def disstudbioclasssec():
    ...
    sec = input("Enter the section:")
    head = ["Admn", "Name", "Father name", "Mother Name", "Class", "Sec"]
    sql = f" select * from student where sclass={sclass} and ssec='{sec}'order by sname"
    cur.execute(sql)
    res = cur.fetchall()
    print(tabulate(res, headers=head))
